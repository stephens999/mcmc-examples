# Computing practical: Introduction to MCMC in R
# 
# Note: anything following a # sign is a comment, and will be ignored by R
# (this should make it easier to cut and paste things)
#
# Another note: the code here is designed to be readable by a beginner, rather than "efficient".
# The idea is that you can use this code to learn about the basics of MCMC, but not as a model for how to program well in R!
#
# Example 1: sampling from an exponential distribution using MCMC
#
# Any MCMC scheme aims to produce (dependent) samples from a ``target" distribution.
# In this case we are going to use the exponential distribution with mean 1
# as our target distribution. So we start by defining our target density.
...
#Having defined the function, we can now use it to compute a couple of values
